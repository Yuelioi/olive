...
const artplayerPlaylist = (options: any) => {
    return function (art: any) {
        // 更新i18n
        const addedI18n = {
            'zh-cn': {
                playlist: '播放列表'
        ...
        art.i18n.update(addedI18n) // 更换分集视频
        const changeVideo = (art: any, index: number) => {
        ...
        const currentEp = options.playlist.findIndex(
            (videoInfo: any) => videoInfo.url === art.option.url
        )
